...
/**
 * This class defines two beans that are referenced in the camel routes. Its
 * properties can be changed using the application.properties file or
...
public class InfinispanAutoConfiguration {
    ...
    public BasicCacheContainer remoteCacheContainer(Environment environment) {
        ...
        String user = environment.getProperty(serviceBaseName + "_SERVICE_USER");
        String password = environment.getProperty(serviceBaseName + "_SERVICE_PASSWORD");
        ...
        clientBuilder.addServer().host(host).port(new Integer(port).intValue()).security().authentication().saslMechanism("PLAIN")
                .username(user).password(password);

        return new RemoteCacheManager(clientBuilder.create(), false);
    }
